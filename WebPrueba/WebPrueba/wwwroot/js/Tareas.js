...
$(document).ready(function () {
    ...
    //let pc = {}; 
    //for (let i = 0; i < 100; i++) {
    //   pc= { id:(i+1), titulo: 'sdsasssssssssssssssssssssssssssssss', descripcion: 'sdssdasssssssssssssssssss', fecha_creacion: '20241101', fecha_vencimiento: '20241101', completada: 1 };
    //    tareas_pendientes.push(pc);
    //}
    consultarTareas();
    ...
    $('#cerra_sesion').on('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
...
function consultarTareas() {
    url =  'http://localhost:5218/api/Tareas/GetTareas'
    $.ajax({
        method: 'GET',
        url: url,
        dataType: 'json'
...
function ActualizaTareas(guardar, tareas) {
    ...
    let operacion = "";
    if (guardar) {
        url = 'http://localhost:5218/api/Tareas/AddTarea';
        operacion = 'POST';
        delete tareas['Id'];
    } else {
        url = 'http://localhost:5218/api/Tareas/UpdateTarea';
        operacion = 'PUT';
    }
    $.ajax({
        method: operacion,
...
function EliminarTareas(tareas) {
    ...
    $.ajax({
        method: 'PUT',
        url: 'http://localhost:5218/api/Tareas/EliminarTarea',
        data: JSON.stringify({
            Id: tareas.id,
            Titulo: tareas.titulo,
            Descripcion: tareas.descripcion,
